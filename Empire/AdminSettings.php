    ...
    public function adminSettings()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if (isset($_POST['empire_sync_ads_txt'])) {
                ...
                update_option(
                    'empire::amp_ads_enabled',
                    isset($_POST['empire_amp_ads_enabled']),
                    false
                );
                update_option(
                    'empire::inject_ads_config',
                    isset($_POST['empire_inject_ads_config']),
                    false
                );
                update_option(
                    'empire::ad_slots_prefill_enabled',
                    isset($_POST['empire_ad_slots_prefill_enabled']),
                    false
                );
        ...
        <div class="wrap">
            ...
            <form method="post">
                <p><label><input type="checkbox" name="empire_enabled"
                                 id="empire_enabled" <?php echo $enabled ? 'checked' : ''; ?>>
                        Empire Integration Enabled</label></p>

                <hr />
                <h3>Empire Settings</h3>
                <p><label>Empire API Key:
                        <input type="text"
                               name="empire_sdk_key"
                               style="width: 355px;"
                               value="<?php echo $sdk_key; ?>" /></label></p>
                <p><label>Empire Site ID:
                        <input type="text"
                               name="empire_site_id"
                               style="width: 355px;"
                               value="<?php echo $site_id; ?>" /></label></p>

                <p><label>Consent Management:
                        <select id="empire_cmp" name="empire_cmp">
                            <option value="">None (WARNING: DO NOT USE IN PRODUCTION)</option>
                            <option value="built-in" <?php
                                echo ( $cmp == 'built-in' ? 'selected="selected"' : '' );
                            ?>>Built In</option>
                            <option value="one-trust" <?php
                                echo ( $cmp == 'one-trust' ? 'selected="selected"' : '' );
                            ?>>One Trust</option>
                        </select>
                    </label></p>
                <p id="one-trust-config" style="display: <?php
                    echo ( $cmp == 'one-trust' ? 'block' : 'none' );
                ?>;">
                    <label>One Trust ID:
                        <input type="text"
                               name="empire_one_trust_id"
                               style="width: 355px;"
                               value="<?php echo $one_trust_id; ?>" />
                    </label>
                </p>
                <script>
                ...
                <p><label><input type="checkbox" name="empire_connatix_enabled"
                                 id="empire_connatix_enabled" <?php
                                    echo $connatix_enabled ? 'checked' : '';
                                    ?>>Connatix Ads Enabled</label></p>
                <p id="connatix-config"
                   style="display: <?php echo ( $connatix_enabled ? 'block' : 'none' ); ?>;">
                    <label>Playspace Player ID:
                        <input type="text"
                               name="empire_connatix_playspace_id"
                               style="width: 355px;"
                               value="<?php echo $connatix_playspace_id; ?>" />
                    </label>
                </p>
                <script>
                ...
                <p><label>% of ads on Empire:
                        <input type="text"
                               name="empire_percent"
                               id="empire_percent"
                               value="<?php echo $empire_test; ?>" />
                    </label></p>
                <p><label>Key-Value for Split Test:
                        <input type="text"
                               name="empire_value"
                               id="empire_value"
                               value="<?php echo $empire_value; ?>" />
                    </label></p>
            ...
            <form method="post">
                <ul>
                    ...
                    foreach ($post_types as $post_type) {
                        ...
                        echo "<li><label>";
                        echo "<input type='checkbox' $checked name='empire_post_types[]' value='" .
                            $post_type . "' /> ";
                        echo $post_type;
                        echo "</label></li>\n";
                    }
